...
describe('requirejs-i18njs', function () {
    ...
    it('should behave as expected in development and return a string for ' +
    ...
    it('should build the file', function (done) {
        exec('node ' + pathBuilder + ' -o ./test/files/build.js',
            function (err) {
                expect(fs.existsSync('./test/files/temp/main.built.js')).to.be
                    .ok();
                done(err);
            });
    });
    ...
    it('should build the file with new delimiters', function (done) {
        exec('node ' + pathBuilder + ' -o ./test/files/build.deli.js',
            function (err) {
                expect(fs.existsSync('./test/files/temp/main.deli.built.js'))
                    .to.be.ok();
                done(err);
            });
    });

    it('should behave as expected in production and returns a function for ' +
        'templates', function (done) {
            requirejs.config({
    ...
    it('should behave as expected in production with new delimiters',
        function (done) {
            requirejs.undef('app');
            requirejs(['main.deli.built'], function () {
                requirejs(['app'], function (app) {
                    expect(app.get('deli.normal')).to.eql('simple');
                    expect(app.get('deli.interpolate', data)).to.be
                        .eql(data.interpolate);
